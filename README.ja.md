...
- **すべてのルールは、レビュアーが実際に受け取るものだけで判定できる
  必要がある。** チェックアウトは行われません。プロンプトに入るのは
  diff、*変更された*ファイルのHEAD時点の全文、baseリビジョンの
  guidanceファイル、そして**切り詰められた**プルリクエスト本文だけで、
  リンクも辿りません。したがって「テストで覆われていない呼び出し箇所を
  指摘せよ」のようなルールは誤爆します。テストファイルは変更されて
  いないためプロンプトに存在せず、「プロンプトに無い」ことは
  「リポジトリに無い」ことの証拠ではないからです。diff自体で決着が
  つくルールを書き、推測したくなる箇所では明示的にそう書いてください。
