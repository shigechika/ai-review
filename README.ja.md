...
4. PR ごとに **sticky コメント1つ**を維持します。1行の verdict、生き残った
   ...
   蒸し返されません。各ラウンドはPRの末尾に新規コメントを投稿し、直前の
   コメントを削除するので、常に最新のレビュー状態はスレッドの最後の
   コメントになります。
5. 2回目以降の push では**新規コミットのみ**をレビューし（compare API に
   よる delta ラウンド）、コード PR へのドキュメントのみの push はスキップ
   し、delta が不完全なときは安全側の full-diff ラウンドに切り替えます。
